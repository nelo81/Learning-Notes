...
举个简单例子，上图的这个结构就是类 D 建立 mro 对象 (tp_mro) 时用到的辅助列表, 图中最右边的 C->B 是类 D 的直接基类列表，其余的话，灰色的部分代表上面白色部分的 mro 列表，下面讲解一下放置的步骤，获得的元素都是从图中这个辅助列表中来
...
4. 遍历 type->bases, 将所有基类放入 type->tp_subclasses 中
