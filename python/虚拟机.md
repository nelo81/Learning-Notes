...
- 创建 PyFrameObject(PyFrame_New)
    ...
    total_memory = f_code->co_nlocals + size(f_code->co_cellvars) + size(f_code->co_freevars) + f_code->co_stacksize
    extras = f_code->co_nlocals + size(f_code->co_cellvars) + size(f_code->co_freevars)
...
STORE_FAST 0: 读取符号表 co->co_varnames 中第 0 个数据，这里是 i, 然后从栈顶 pop 拿出刚才压入的 1，最后设值到 co->co_varnames\[0\], f->f_locals 存放着所有局部变量，包括这个 co_varnames
...
2. LOAD_FAST 将 list 放到栈顶，然后 GET_ITER 从栈顶上的 list 获取迭代器，再放到栈顶
3. FOR_ITER: 从栈顶获取迭代器，通过 iternext 将一个个数组元素压入栈顶，然后进行下一步操作，这里是 STORE_FAST，就是将取出来的这个栈顶的元素赋值到 i，然后后面是输出；
